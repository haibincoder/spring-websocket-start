package com.imonit.AiTeaching.config;

import com.imonit.AiTeaching.controller.TeachingController;
import com.imonit.AiTeaching.service.Handshake;
import com.imonit.AiTeaching.service.SocketHandler;
...
public class WebSocketConfig implements WebSocketConfigurer {
    ...
    public void registerWebSocketHandlers(WebSocketHandlerRegistry registry) {
        ...
        registry.addHandler(handler, "/connect").addInterceptors(handshake).setAllowedOrigins("*");
        //部分 不支持websocket的访问链接,允许跨域
        registry.addHandler(handler, "/sockjs/connect").addInterceptors(handshake).setAllowedOrigins("*").withSockJS();
    }
}
